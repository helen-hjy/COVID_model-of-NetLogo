;; Full Name: Jiayu Han
;; Student ID: 1164280
;; Updated from epiDEM model by
;; Yang, C. and Wilensky, U. (2011). NetLogo epiDEM Basic model.
;; http://ccl.northwestern.edu/netlogo/models/epiDEMBasic.
...
[
  ...
  ;; new var below
  death-rate            ;; death-rate of covid (0.019 in AUS)
  initial-people        ;; inialized population
  recovery-chance       ;; rate of recovery
  average-recovery-time ;; average recovery time period (in days)
...
[
  ...
  ;; new var below
  death?              ;; If true, the person is dead and cannot spread the covid
  wearing-mask?       ;; If true, this person wears mask all time
  nb-wearing-mask     ;; Number of people wearing mask
...
to setup
  ...
  set initial-people 5000              ;; set population
  ;; actual data from COVID-19
  set recovery-chance 97               ;; recovery-chance of 97%
  set average-recovery-time 28         ;; average recovery time in days
  set death-rate 2                     ;; death-rate of covid (0.019 in AUS)

  ;; setup individuals state variables
  setup-people
...
to setup-people
  ...
  [
    ...
    ;;----- setup first infected cases -----
    ;; Each individual has infection-chance of starting out infected.
    ...
    ;;---- setup masked people ----
    if(random 100 < rate-of-wearing-mask)
    [
      set wearing-mask? true
      ;; add 1 to nb-wearing-mask
      set nb-wearing-mask (nb-wearing-mask + 1)
    ]
    assign-color
  ]
end
...
;; Yellow is wearing mask and susceptible
;; Shape X means this person is dead
to assign-color  ;; turtle procedure
  ...
  ;; set color for masked people
  if wearing-mask?
    [ set color yellow
     ]
  ;; set shape and color for people cured and wearing mask
  if cured? and wearing-mask?
    [
      set shape "dot"
      ]

  ;; set shape and color for death people with x
  if death?
    [ set shape "x"
      set color 7 ]
...
to go
  ;; ask dead people stop moving
  ask turtles with [death?]
    [ assign-color
      stop ]
...
to clear-count
  set nb-infected 0
  set nb-recovered 0
  set nb-wearing-mask 0
end

;; Infection can occur to any susceptible person nearby
to infect
  ;; create agentset for 8 surrounding patches (neighbors)
  ;; that are susceptible
  let nearby-uninfected (turtles-on neighbors)
  with [ not infected? and not cured? ]

  ;; set masked people to agentset
  let nearby-mask (turtles-on neighbors)
  with [ wearing-mask? ]

  ;; if nobody wear masks
  ifelse nearby-mask = nobody
  [
    ;; if there's any susceptible nearby
    if nearby-uninfected != nobody
    [
      ask nearby-uninfected
      ;set random rate of infection to this person
      [
        if random-float 100 < infection-chance
        [
         set infected? true
         ;get total num of infected turtles
         set nb-infected (nb-infected + 1)
        ]
      ]
    ]
  ]
  ;; else if somebody wears mask
  [
    ;; if there's any susceptible nearby
    if nearby-uninfected != nobody
    [
      ask nearby-uninfected
      [ ;; set lower rate of infection to this person
        if random-float 100 < (infection-chance - 10 )
        [
         set infected? true
         ;get total num of infected turtles
         set nb-infected (nb-infected + 1)
...
to maybe-recover
  ...
  [
    ;; this person may be recovered
    if random-float 100 < recovery-chance
    [ set infected? false
      set cured? true
      ;; count num of recovered
...
"Dead" 1.0 0 -7500403 true "" "plot count turtles with [ death? ]"
